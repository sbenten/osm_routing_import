CREATE FUNCTION loadtravelineeastmidlands(files character varying[]) RETURNS void
    LANGUAGE plpgsql
    AS $$
DECLARE
rec record;
fileId integer;
/*
Import the East Midlands files
...
Due to minor differences in the XML the East Midlands data which is allowed by the schema
...
BEGIN
	...
	LOOP 	
		...
		INSERT INTO traveline_vehicle_journeys (file_id, vehicle_journey_code, vehicle_service_code, line_ref, service_code, departure_time, days_week)
		SELECT  
			fileId, --No operator code
...
ALTER FUNCTION loadtravelineeastmidlands(files character varying[]) OWNER TO postgres;