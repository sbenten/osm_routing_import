CREATE FUNCTION splitwaysbyflowsamplesource() RETURNS void
    LANGUAGE plpgsql
    AS $$
DECLARE
	rec record;
BEGIN
	...
	WITH x AS (
		SELECT ST_SetSRID(ST_Union(o.point), 27700) AS sources, o.way_id, o.way_geom
		FROM (			
			SELECT s.*, 
			...
			FROM (
				SELECT i.*, 
					ST_LineLocatePoint((SELECT geom FROM ways_clean WHERE id = i.way_id), i.source) AS way_pos
				FROM (
					SELECT id, 
					source,
					nearestdriveablewaytopoint(source) :: int4 as way_id
	...
	SELECT x.way_id, 
		...
		*/
		ST_SetSRID((rc_split_line_by_points(x.way_geom, x.sources, 0.000001)).geom, 27700)
	FROM x;	
...
ALTER FUNCTION splitwaysbyflowsamplesource() OWNER TO postgres;