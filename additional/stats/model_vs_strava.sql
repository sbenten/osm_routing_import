...
BEGIN
	...
	UPDATE sheffield.ways_clean SET total_count = x.total_count
	FROM (
		SELECT resedgeid, COUNT(1) AS total_count
		FROM sheffield.vw_batch_run_results_base
		GROUP BY resedgeid
	) x
	WHERE sheffield.ways_clean.id = x.resedgeid;

	UPDATE sheffield.ways_clean SET bike_count = x.bike_count
	FROM (
	...
	UPDATE sheffield.ways_clean SET walk_count = x.walk_count
	FROM (
	...
	FROM (
		...
		FROM sheffield.strava
		GROUP BY osm_id
	) x	
	WHERE sheffield.ways_clean.osm_id = (x.osm_id :: character varying);
	...
	UPDATE sheffield.ways_clean SET 
...
*/
SELECT max(bike_count), max(strava_commute_count), max(total_accidents), max(cyclist_accidents) FROM sheffield.ways_clean;

SELECT ((strava_commute_count / 13815) * 100) :: integer FROM sheffield.ways_clean; --13815 = maximum commuter count in Strava data
